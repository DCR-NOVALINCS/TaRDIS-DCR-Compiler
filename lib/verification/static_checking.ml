...
type ('a, 'b) leakError = SAT | Unknown of 'a | UNSAT of 'b
  ...
  let rec aux lst acc =
    ...
    | x :: xs -> (
        ...
        | Unknown s -> (
            ...
            match acc with
            | Unknown s' ->
                (* print_endline "Both unknown"; print_endline @@
                   unparse_cnf_formula s; print_endline @@ unparse_cnf_formula
                   s'; *)
                (* print_endline @@ unparse_cnf_formula (cnf_or s s'); *)
                (* print_endline @@ unparse_cnf_formula (cnf_and s s'); *)
                let p : cnf_formula = cnf_or s s' in
                aux xs (Unknown p)
            | UNSAT _ ->
                (* print_endline @@ unparse_cnf_formula s; *)
                Unknown s
            | SAT -> aux xs acc)
        | SAT -> SAT)
  in
...
(* 2 mapas 1 uuid event -> condition
   2 uuid relation -> string (nome do
      triggers)
*)
let global_label_SC : security_level TreeMap.t ref = ref TreeMap.empty
let counter = ref 0 (* added to trigger*)

let conditionMap : expr TreeMap.t ref =
  ref TreeMap.empty (* uuid event -> condition*)

let spawnMap : string TreeMap.t ref =
  ref TreeMap.empty (* uuid spawn -> trigger id*)
...
let reset_references new_program =
  let return_map = conditionMap in
  let return_map2 = spawnMap in
  global_label_SC := TreeMap.empty;
  conditionMap := TreeMap.empty;
  spawnMap := TreeMap.empty;
  counter := 0;
  Ok new_program
...
let find_env env depth =
  let rec aux env depth =
    if depth = 0
    then Ok env
    else if List.length env = 0
    then Error (Nowhere, "Error in find_env")
...
module Local = struct
  ...
  type ('a, 'b) resultExpr = Ok of 'a | Error of 'b | Unknown
end

let rec evalExpr expr env : Local.evalExpr =
  match expr.data with
  | True -> Bool true
  | False -> Bool false
  | IntLit n -> Int n
  ...
  | PropDeref (e, prop) -> (
      let rec evalProp e prop env : ('a, 'b) Local.resultExpr =
        ...
        | EventRef e -> (
            match find_flat_opt e.data env with
        ...
        | PropDeref (e, prop2) -> (
            ...
            | Ok (exp, env2) -> (
                ...
                | Record rs -> (
                    ...
                    with
                    | None ->
                        Error (e.loc, "Error in propDeref not found record")
                    | Some s ->
...
let rec check_static_information_security program =
  let global_program : spawn_program ref =
    ref { events = []; relations = [] }
  in
  build_lattice program.roles program.security_lattice (ref TreeMap.empty)
    (ref TreeMap.empty)
  >>= fun (lattice, params) ->
  retrive_security_of_events
    (program.spawn_program.events, program.spawn_program.relations)
    (empty, lattice, params, global_program)
  >>= fun _ ->
  check_security_graph
    (program.spawn_program.events, program.spawn_program.relations)
    (empty, lattice, params, empty)
  >>= fun _ -> reset_references program
  ...
  let build_lattice_level (flw : flow_relation')
    ...
    | None ->
        Error
          [
            ( left.loc
            , "Error in lattice flow " ^ left.data ^ " -> " ^ right.data )
          ]
  in
...
(** Function used to retrieve the security levels of all events in program. 
  To be used in data of events. *)
and retrive_security_of_events (events, crs)
    (env, lattice, sec_parms, new_program) =
  let process_events (event : event') (env, lattice, sec_parms, new_program) =
    ...
    if all_levels_const event_data
    then
      Ok
        ( bind id.data (mkNodeEvent event_data env) env
        , lattice
        , sec_parms
        , new_program )
    else
      Error
        [
          ( event.loc
          , "Error in typechecking security level of event "
            ^ id.data
            ^ ":"
            ^ label.data )
        ]
  ...
  let process_spawn_relation cr (env, level_map, sec_parms, new_program) =
    ...
    | SpawnRelation (_, _, prog) ->
        ...
        >>= fun (env2, level_map2, sec_params2, new_program) ->
        Ok (end_scope env2, level_map2, sec_params2, new_program)
  in
  fold_left_error
    ...
    events
  >>= fun tuple ->
  fold_left_error (fun tuple cr -> process_spawn_relation cr tuple) tuple crs
...
and get_security_of_expr expr env lattice sec_params simbolic_env =
  ...
  | BinaryOp (e1, e2, _) -> (
      ...
      | Ok pc1 -> (
          ...
          | Error e ->
              Error ((e2.loc, "Error in exp node" ^ unparse_expr e2) :: e)
          | Ok pc2 ->
              Ok
                (get_levels_from_SC_List filter_higher_levels pc1 pc2 lattice
                   sec_params env)))
  | UnaryOp (e, _) -> get_security_of_expr e env lattice sec_params simbolic_env
  | EventRef id -> (
  ...
  | Record list ->
      let n_list =
        List.map
          (fun r ->
            get_security_of_expr (snd r.data) env lattice sec_params
              simbolic_env)
          list
      in
...
1. Verify events 
2. Verify relations *)
and check_security_graph (events, crs) ctxt =
  fold_left_error
      ...
      | Ok security_level -> (
          ...
          | None, _ ->
              Error
                [
                  ( cr.loc
                  , "Error in control relation finding event1 in "
                    ^ e1.data
                    ^ "flows"
                    ^ e2.data )
                ]
          | _, None ->
              Error
                [
                  ( cr.loc
                  , "Error in control relation finding event2 in "
                    ^ e1.data
                    ^ "flows"
                    ^ e2.data )
                ]
          | Some node1, Some node2 -> (
              let verify_relations_events () =
                match
                  check_less_or_equal_security_level node1.security_list
                    node1.env node2.security_list node2.env sec_params lattice
                    true
                with
                | UNSAT e ->
                    Error
                      (( cr.loc
                       , "Does not flow in control relation "
                         ^ e1.data
                         ^ "("
              ...
              in
              match
                check_less_or_equal_security_level node1.security_list node1.env
                  security_level env sec_params lattice false
              with
  ...
  | SpawnRelation (event, exp, prog) -> (
      ...
      | Ok security_list -> (
          ...
          | Some node -> (
              ...
              in
              match
                check_less_or_equal_security_level node.security_list node.env
                  security_list env sec_params lattice false
              with
              | UNSAT e ->
                  Error
                    (( exp.loc
...
and check_security_event event (env, lattice, sec_params, simbolic_env) =
  let check_security_trigger (event : event') env lattice =
    ...
    | Some (data, depth) -> (
        if depth != 0
        then Error [ (event.loc, "Error: trigger does not exists") ]
        else
          match
            check_less_or_equal_security_level data.security_list data.env
              event.data.security_level.data env sec_params lattice true
          with
          | UNSAT e ->
              Error
                ((event.loc, "Error in security with event and trigger") :: e)
          | SAT -> Ok ()
          | Unknown e ->
              print_endline @@ unparse_cnf_formula e;
              event.ifc := Some true;
  ...
  let static_check_security_of_data_expr event env lattice sec_params
    ...
    let rec check_security_of_type_expr type_expr lattice params =
      ...
      | EventRefTy (s, _) -> (
          ...
          | Some node ->
              Ok
                (get_levels_from_SC_List filter_higher_levels node [] lattice
                   sec_params env))
      | EventTy s -> (
          match TreeMap.find_opt s !global_label_SC with
          | Some node ->
      ...
      | RecordTy r -> (
          ...
          | Ok get_list ->
              Ok
                (get_levels_from_SC_List filter_higher_levels get_list []
                   lattice sec_params env))
      | ListTy _ ->
    ...
    | Input ty_expr -> (
        ...
        | Error e ->
            Error
              (( ty_expr.loc
               , "Error in expression of event "
                 ^ (fst event.info.data).data
                 ^ ":"
                 ^ (snd event.info.data).data )
              :: e)
        | Ok security_lvl -> (
            match
              check_less_or_equal_security_level event.security_level.data env
                security_lvl env sec_params lattice false
            with
            | SAT -> Ok ()
            | UNSAT e ->
                Error
                  ...
                  :: e)
            | Unknown e ->
                print_endline @@ "Aqui unknow trigger";
                print_endline @@ unparse_cnf_formula e;
  ...
  let static_checking_security_param (security_event : security_level) loc env
    ...
    fold_left_error
      (fun (acc, env, lattice, params) (lvl : sec_label') ->
        ...
        | Error e -> Error e)
      ([], env, lattice, params) security_event
    >>= fun (sec2, env2, lattice2, params2) ->
    Ok
      (get_levels_from_SC_List filter_higher_levels sec2 [] lattice2 params2
         env2)
  in
  ...
  | Ok () -> (
      match
        static_check_security_of_data_expr event.data env lattice sec_params
          simbolic_env
      with
      | Error e ->
          Error
            (( event.data.info.loc
             , "Error checking ifc levels in event "
               ^ id.data
               ^ ":"
               ^ label.data )
            :: e)
      | Ok () -> (
          ...
          | Error e ->
              Error
                (( event.data.info.loc
                 , "Error checking wellformedness in event "
                   ^ id.data
                   ^ ":"
                   ^ label.data )
                :: e)
          | Ok _ -> (
              ...
              match
                static_checking_security_param event.data.security_level.data
                  Nowhere new_env lattice sec_params
              with
              | Error e ->
                  Error
                    (( event.data.info.loc
              ...
              | Ok list -> (
                  match
                    check_less_or_equal_security_level
                      event.data.security_level.data env list env sec_params
                      lattice false
                  with
...
and check_less_or_equal_security_level (event_security : security_level)
    (env : node t) (exp_security : security_level) (env2 : node t) sec_params
    lattice relation =
          ...
          List.map
            (fun node_1 ->
              ...
              then
                ...
                let p =
                  depth_first_search node_2 node_1 !lattice [] sec_params env2
                    env
                in
                p)
            event_security
...
and depth_first_search (node1 : sec_label') (node2 : sec_label')
    ...
    (env : node t) env2 =
  if List.mem (fst node1.data).data visited
  then SAT
  else if (fst node1.data).data = (fst node2.data).data
  then compareSecurityLevels node1 node2 params env env2
  else
    let rec depth_first_search' (label1 : string) (label2 : string)
        (lattice : string list TreeMap.t) (visited : string list)
        (path : string list) =
      if List.mem label1 visited
      then SAT
      else if label1 = label2
      then
        compareSecurityLevels
          (annotate (annotate label1, []))
          node2 params env env2
      else
        ...
        | Some l ->
            or_list
              (List.map
                 (fun l' ->
                   depth_first_search' l' label2 lattice (label1 :: visited)
                     (label1 :: path))
                 l)
        | None -> SAT
    in
    depth_first_search' (fst node1.data).data (fst node2.data).data lattice
      visited []
      ...
      List.map
        (fun param ->
          ...
          | Parameterised exp1, Parameterised exp2 -> (
              match (evalExpr exp1 env, evalExpr exp2 env2) with
              | Int n1, Int n2 ->
                  if n1 == n2
                  then SAT
                  else
              ...
              | Error s, _ ->
                  UNSAT
                    [
                      ( exp1.loc
                      , "Error fst expr in comparing exprs: "
                        ^ unparse_expr exp1
                        ^ " = "
                        ^ unparse_expr exp2 ); s
                    ]
              | _, Error s ->
                  UNSAT
                    [
                      ( exp2.loc
                      , "Error snd expr in comparing exprs: "
                        ^ unparse_expr exp1
                        ^ " = "
                        ^ unparse_expr exp2 ); s
                    ]
              | Unknown, Unknown ->
                  Unknown
                    [
                      [
                        Positive
                          (CnfSymEq (unparse_expr exp1, unparse_expr exp2))
                      ]
                    ]
              | Unknown, Int s ->
              ...
              | Unknown, Bool s ->
                  Unknown
                    [ [ Positive (CnfEq (unparse_expr exp1, BoolLit s)) ] ]
              | Unknown, String s ->
                  Unknown
                    [ [ Positive (CnfEq (unparse_expr exp1, StringLit s)) ] ]
              | Int s, Unknown ->
                  Unknown [ [ Positive (CnfEq (unparse_expr exp2, IntLit s)) ] ]
              | Bool s, Unknown ->
                  Unknown
                    [ [ Positive (CnfEq (unparse_expr exp2, BoolLit s)) ] ]
              | String s, Unknown ->
                  Unknown
                    [ [ Positive (CnfEq (unparse_expr exp2, StringLit s)) ] ]
              | _ ->
                  UNSAT
                    [
                      ( exp1.loc
...
and get_levels_from_SC_List filter_function (pc_list : security_level)
    (pc2 : security_level) lattice params (env : node t) =
  ...
  let noRepeatPc =
    List.fold_left
      (fun acc l ->
        if List.exists
             (fun l' ->
               match compareSecurityLevels l l' params env env with
               | SAT -> true
               | Unknown _ -> false
               | UNSAT _ -> false)
             acc
  ...
  let rec filter_levels (filtered : sec_label' list) = function
    ...
    | l :: ls ->
        let filtered' =
          List.filter
            (fun l' ->
              match depth_first_search l l' !lattice [] params env env with
              | SAT -> false
              | _ -> true)
            filtered
        in
...
and concat_list list loc lattice sec_params env :
  ...
  | Ok result ->
      Ok
        (get_levels_from_SC_List filter_higher_levels result [] lattice
           sec_params env)
