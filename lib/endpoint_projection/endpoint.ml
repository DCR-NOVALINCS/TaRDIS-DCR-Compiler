...
let rec unparse_events ?(indent = "") (events : event list) =
  ...
  let rec unparse_event (e : event) =
    ...
    let rec unparse_info () = Printf.sprintf "(%s : %s)" e.id e.label
    and unparse_participants (roles : user_set_expr' list) =
      List.map
        (function
          | RoleExpr role_expr' ->
            let name', params = role_expr'.data in
            List.map
              (fun (param' : user_set_param_val' Choreo.named_param') ->
                let param_name', value' = param'.data in
                begin
                  ...
                  | Expr expr' ->
                    sprintf
                      "%s=%s"
                      param_name'.data
                      (Frontend.Unparser.unparse_expr expr')
                  | Any -> sprintf "%s=*" param_name'.data
                end)
              params
            |> String.concat ", "
            |> sprintf "%s(%s)" name'.data
          | Initiator event_id' -> sprintf "@Initiator(%s)" event_id'.data
          | Receiver event_id' -> sprintf "@Receiver(%s)" event_id'.data)
