...
(* let replace str target replacement =
   let open Str in
   global_replace (regexp_string target) replacement str *)
...
(* let unparse_to_file_with_blob_tmpl dest_file blob_tmpl_str tmpl_context =
   let oc = open_out dest_file in
   List.iter (fun (k, v) ->
     Printf.fprintf oc "%s\n" @@ replace blob_tmpl_str (Printf.sprintf "// {{%s}}" k) v)
     tmpl_context;
   close_out oc *)
...
let process_choreography lexbuf =
  ...
  (* Static_checking.check_static_information_security program >>= fun () -> *)
  ...
  (* TODO [post-demo] have projections return something more friendly than
   the entire projection context - need to check what's needed first *)
  Projections.project program >>= fun _projection_ctxts -> Ok ()
(*   let babel_unparsed =
     List.map
       (Babel.Unparser.babel_unparse event_types_by_label)
       projection_ctxts
   in *)
(* TODO -> let unparsed_projections = .... *)
(* let babel_unparsed =
     Babel.Translate.translate projections
     |> Babel.Unparser.unparse ~indent:"\t  "
   in *)

(* TODO -> List.iter unparsed_projections unparse_projection_to_file *)
(* unparse_to_file_with_blob_tmpl
   "output_babel.java"
   ([%blob "resources/input_babel.java"])
   [ ("code", babel_unparsed) ]; *)
...
(* Projections.project (prog.events, prog.relations) >>= fun projections ->
   Babel.Translate.translate projections >>= fun babel_ctxt ->
   let unparsed_prog = Unparser.unparse_prog ~indent:"" ~abbreviated:false prog in
   let babel_unparsed = Babel.Unparser.unparse ~indent:"\t\t" babel_ctxt in *)
